...
public class MemberOkCommand implements CommandService {
	...
	public String processStart(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		// 뷰에서 정보를 서버로 가져오기
		req.setCharacterEncoding("UTF-8");		//post로 request해서 받아오면 한글이 깨진다.
		...
		vo.setInterest(req.getParameterValues("interest")); //checkbox는 배열이라
		
		// DB에 추가
		MemberDAO dao = MemberDAO.getInstance();
		int cnt = dao.setNewMember(vo);  //0 -> 실패,  1 -> 성공
		
		// 뷰로 보내기
		req.setAttribute("result", cnt); //회원가입 결과여부를 attribute에 심어서 보낸다
